...
filepath = "/Users/pmxfp1/Library/CloudStorage/OneDrive-TheUniversityofNottingham/Newton Syncropatch/Frankie_Experiments_Nov_Dec_2023/30112023_MW1/milnes_protocol_13.10.42/"
json_file = "milnes_protocol_13.10.42"
...
outloc = "/Users/pmxfp1/Desktop/PhD-Frankie/Code/2024/Jan/test"
...
drug = "verapamil"
concs = [0.1, 0.3, 1]
col_pairs = [['19', '20'], ['21', '22'], ['23', '24']]
colours = ['#1f77b4', '#ff7f0e', '#2ca02c']

### specify number of sweeps to plot, min current, first drug sweep, and crop index
num_swps = 10
min_curr = 80
first_drug = 6
crop_ind = 1
...
exc_wells = ['B20', 'C19', 'H20', 'J20', 'M19', 'O20', 'P20', 'B24', 'L24', 'N24', 
             'A23', 'A24', 'D23', 'E23', 'F24', 'K23', 'M23', 'F22', 'K22', 'N22',
             'O22']
...
if protocol:
    ...
    ax.axvline(pulse[0], color = 'r', linestyle = '--', linewidth = '0.7', label = 'pulse bounds')
...
for conc, cols in zip(concs, col_pairs):
    if leak_corr:
        ...
        for well in filt_QC.keys():
            if well[1:] in cols and well not in exc_wells: # filter out additional wells that don't pass manual QC
                lc.fit_linear_leak(trace_data, well, first_drug - 2, 
                                   ramp_bounds = [2*r for r in ramp], plot = True, output_dir = outdir)
                lc.fit_linear_leak(trace_data, well, first_drug + 4, 
                                   ramp_bounds = [2*r for r in ramp], plot = True, output_dir = outdir)
    ...
    if current:
        ...
        for well in filt_QC.keys():
            if well[1:] in cols and well not in exc_wells:
                fig, ax = plt.subplots()
                ax.plot(1000*ts, lc_data[well][first_drug - 2], label = 'control')
                ax.plot(1000*ts, lc_data[well][first_drug], label = 'sweep 1', alpha = 0.5)    
                ax.plot(1000*ts, lc_data[well][first_drug + 4], label = 'sweep 5', alpha = 0.5)
                ax.plot(1000*ts, lc_data[well][first_drug + 19], label = 'sweep 20', alpha = 0.5)
    ...
    for well in filt_QC.keys():
        if well[1:] in cols and well not in exc_wells:
            multi_sweeps = []
            last = 0
            control = lc_data[well][first_drug - 2][2*pulse[0]:2*pulse[1]]
            all_control.append(control)
            for i in np.arange(first_drug, num_swps+first_drug):
                sweep = lc_data[well][i][2*pulse[0]:2*pulse[1]]
...
if all_block:
    ...
    if save_csvs:
        ### save csvs
        for conc in concs:
            with open(f"{outloc}/{drug}/{conc}/{drug}_conc_{conc*1000}_Milnes.csv", 'w') as f:
